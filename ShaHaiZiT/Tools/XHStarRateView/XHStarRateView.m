...
@property (nonatomic, copy) NSString *defaultPicStr;
@property (nonatomic, copy) NSString *selectedPicStr;
...
-(instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        _selectedPicStr = @"comment_show_selected";
        _defaultPicStr = @"comment_show_praise";
        [self createStarView];
    }
    return self;
}
...
#pragma mark - block方式
-(instancetype)initWithFrame:(CGRect)frame isT:(BOOL)isT finish:(finishBlock)finish{
    if (self = [super initWithFrame:frame]) {
        _numberOfStars = 5;
        _rateStyle = WholeStar;
        if (isT) {
            _selectedPicStr = @"comment_t_selected";
            _defaultPicStr = @"comment_t_praise";
        }else{
            _selectedPicStr = @"comment_selected";
            _defaultPicStr = @"comment_praise";
        }
...
-(void)createStarView{
    ...
    self.foregroundStarView = [self createStarViewWithImage:_selectedPicStr];
    self.backgroundStarView = [self createStarViewWithImage:_defaultPicStr];
