...
/**转换成当前比例的数*/
#define Ratio(x) ((int)((x) * (SCREEN_WIDTH/375.0)))
#define SELDescriptionFont 14
...
+ (void)showUpdateAlertWithVersion:(NSString *)version Descriptions:(NSArray *)descriptions isForce:(BOOL)isForce
{
    ...
    SELUpdateAlert *updateAlert = [[SELUpdateAlert alloc]initVersion:version description:description isForce:isForce];
    [[UIApplication sharedApplication].delegate.window addSubview:updateAlert];
}


- (instancetype)initVersion:(NSString *)version description:(NSString *)description isForce:(BOOL)isForce
{
    self = [super init];
    if (self) {
        ...
        [self _setupUIWithIsForce:isForce];
    }
    return self;
}

- (void)_setupUIWithIsForce:(BOOL)isForce
{
    ...
    updateIcon.image = BGImage(@"headImg_placeholder");
    ...
    descTextView.textColor = kApp333Color;
