...
+(instancetype)sharedWithImages:(NSArray *)imageNames{
    images = imageNames;
    isScrollOut = YES;
    launch = [[LaunchIntroductionView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if ([self isFirstLauch]) {
            ...
            BGSetUserDefaultObjectForKey([NSDate date], @"UpdateOld_date");
            UIStoryboard *story;
            if (storyboard) {
...
-(void)createScrollView{
    ...
    jumpBtn.frame = CGRectMake(SCREEN_WIDTH-90, SafeAreaTopHeight-34, 70, 26);
    [jumpBtn.titleLabel setFont:kFont(15)];
    jumpBtn.layer.cornerRadius = 5;
    jumpBtn.clipsToBounds = YES;
    jumpBtn.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.4];
