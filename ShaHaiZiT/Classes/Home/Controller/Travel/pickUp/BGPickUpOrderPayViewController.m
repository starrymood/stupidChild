//
//  BGPickUpOrderPayViewController.m
//  shzTravelC
//
...
@interface BGPickUpOrderPayViewController ()
...
@implementation BGPickUpOrderPayViewController
-(UIView *)backView{
    if (!_backView) {
...
-(void)setbottomViewLayout{
    ...
    _payPriceLabel.font = [UIFont fontWithName:@"Arial" size:16];
...
-(void)updateSubViewsWithModel:(BGAirPayInfoModel *)model{
    ...
    _flightNumTextField.text = model.flight_number;
    _personNameTextField.text = model.contact;
    _packageNumTextField.text = model.baggage_number;
    _destinationTextField.text = model.destination;
    _arriveTimeTextField.text = [Tool dateFormatter:model.start_time.doubleValue dateFormatter:@"yyyy-MM-dd HH:mm"];
