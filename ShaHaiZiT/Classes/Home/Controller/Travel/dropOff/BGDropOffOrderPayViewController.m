//
//  BGDropOffOrderPayViewController.m
//  shzTravelC
//
...
#import "BGDropOffOrderPayViewController.h"
...
@interface BGDropOffOrderPayViewController ()
...
@property (weak, nonatomic) IBOutlet UITextField *desAirportTextField;
...
@implementation BGDropOffOrderPayViewController
...
-(void)setbottomViewLayout{
    ...
    sLab.frame = CGRectMake(23, 17, 80, 15);
    sLab.font = kFont(13);
    sLab.textColor = kApp666Color;
    sLab.text = @"实际支付：";
    [_bottomView addSubview:sLab];
    
    self.payPriceLabel = UILabel.new;
    _payPriceLabel.frame = CGRectMake(23+80, 17, SCREEN_WIDTH-23-80-141, 15);
    _payPriceLabel.textAlignment = NSTextAlignmentLeft;
    _payPriceLabel.font = [UIFont boldSystemFontOfSize:17];
    _payPriceLabel.textColor = UIColorFromRGB(0xFF5656);
    _payPriceLabel.text = @"¥0.00";
    [_bottomView addSubview:_payPriceLabel];
    
    self.payBtn = [UIButton buttonWithType:(UIButtonTypeCustom)];
    _payBtn.frame = CGRectMake(SCREEN_WIDTH-141, 0, 141, 49);
    [_payBtn.titleLabel setFont:[UIFont boldSystemFontOfSize:15]];
...
-(void)updateSubViewsWithModel:(BGAirPayInfoModel *)model{
    ...
    _desAirportTextField.text = model.airport_name;
    _personNameTextField.text = model.contact;
    _packageNumTextField.text = model.baggage_number;
    _originTextField.text = model.departure;
    _originTimeLabel.text = [Tool dateFormatter:model.start_time.doubleValue dateFormatter:@"yyyy-MM-dd HH:mm"];
    ...
    [self.carImgView sd_setImageWithURL:[NSURL URLWithString:model.car_pictures[0]] placeholderImage:BGImage(@"img_cycle_placeholder")];
...
-(void)changeViewFrame{
    self.contentCenterY.constant = (_orderCouponView.y+_orderCouponView.height-SCREEN_HEIGHT+SafeAreaTopHeight+SafeAreaBottomHeight+49+6+self.noteViewHeight.constant+self.serviceViewHeight.constant-30-100)*0.5;
