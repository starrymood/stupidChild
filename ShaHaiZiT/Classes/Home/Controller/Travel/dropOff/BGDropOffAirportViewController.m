//
//  BGDropOffAirportViewController.m
//  shzTravelC
//
...
@interface BGDropOffAirportViewController ()<UITextViewDelegate,pickViewDelegate,THDatePickerViewDelegate>
...
@implementation BGDropOffAirportViewController
...
-(void)loadSubViews{
    ...
    self.desAirportTextField.maxLenght = 40;
...
//        self.carImgView.contentMode = UIViewContentModeScaleAspectFit;
    ...
    self.desAirportTextField.text = _fModel.airport_name;
...
-(void)uploadInfoAction:(UIButton *)sender{
    ...
    if (_desAirportTextField.text.length<1) {
        [WHIndicatorView toast:@"请填写送达机场"];
        return;
    }
    if (_originTextField.text.length<1) {
        [WHIndicatorView toast:@"请填写出发地"];
        return;
    }
    if ([_originTimeLabel.text isEqualToString:@"选择出发时间"]) {
        [WHIndicatorView toast:@"请选择出发时间"];
        return;
    }
    ...
    if (_noteTextView.text.length<1) {
     ...
     NSString *timeStampStr = [Tool timeString:_originTimeLabel.text withFormatte:@"YYYY-MM-dd HH:mm"];
...
//     NSMutableDictionary *param = [[NSMutableDictionary alloc] init];
//     [param setObject:_product_id forKey:@"product_id"];
//     [param setObject:@"放鹤路1088号" forKey:@"departure"];
//     [param setObject:timeStampStr forKey:@"start_time"];
...
//     [param setObject:@"飞雪连天射白鹿,笑书神侠倚碧鸳.京东包车景点简介京东包车景点简介京东包车景点简介 京东包车景点简介京东包车景点简介京东包车景点简介 京东包车景点简介京东包车景 拷贝" forKey:@"remark"];
    ...
    [BGOrderTravelApi uploadPreInfo:param succ:^(NSDictionary *response) {
        ...
        BGDropOffOrderPayViewController *payVC = BGDropOffOrderPayViewController.new;
...
- (void)datePickerViewSaveBtnClickDelegate:(NSString *)timer {
    ...
    self.originTimeLabel.text = timer;
    [UIView transitionWithView:self.backView duration:0.3 options:0 animations:^{
