//
//  BGHomePreSearchViewController.m
//  ShaHaiZiT
//
...
#define kCachePath ([NSHomeDirectory()stringByAppendingPathComponent:@"Documents/history6"])

@interface BGHomePreSearchViewController ()<EVNCustomSearchBarDelegate>
/**
...
@implementation BGHomePreSearchViewController
...
-(NSArray *)readNSUserDefaults {
    NSArray *historyArr;
    historyArr = [NSKeyedUnarchiver unarchiveObjectWithFile:kCachePath];
    if (nil == historyArr) {
        historyArr = @[];
    }
...
- (void)pushViewWithTitle:(NSString *)title isPerson:(BOOL)isPerson{
    ...
    // 跳转新界面
    BGHomeSearchViewController *listVC = BGHomeSearchViewController.new;
    listVC.name = title;
