...
#define BtnHeight (6+45)
@interface BGStrategyAViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,BGWaterFallLayoutDelegate>
...
@implementation BGStrategyAViewController
-(UIView *)noneView {
    if (!_noneView) {
...
- (void)setupLayoutAndCollectionView{
    ...
    // 创建collectionView
    UICollectionView *collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-SafeAreaTopHeight-SafeAreaBottomHeight-kTabBarH-BtnHeight) collectionViewLayout:waterFallLayout];
...
- (CGFloat)waterFallLayout:(BGWaterFallLayout *)waterFallLayout heightForItemAtIndexPath:(NSUInteger)indexPath itemWidth:(CGFloat)itemWidth{
    ...
    NSInteger lineNum = [self needLinesWithWidth:(itemWidth-16) text:model.post_title];
...
//    DLog(@"\n>>>[collectionView didSelectItem]:%ld",(long)indexPath.item);
    ...
    if (model.type.intValue == 2) {
        ...
        for (BGStrategyModel *model in self.cellDataArr) {
            if (model.type.intValue == 2) {
                [dataArr addObject:model];
            }
        }
        detailVC.dataArr = [NSArray arrayWithArray:dataArr];
        detailVC.pageNum = self.pageNum;
        detailVC.isSingle = NO;
        detailVC.classification_id = self.classification_id;
        for (int i = 0; i<dataArr.count; i++) {
            BGStrategyModel *sModel = dataArr[i];
            if ([model.file_url isEqualToString:sModel.file_url]) {
                [detailVC playTheIndex:i];
                break;
            }
        }
