...
@interface BGMyFansViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation BGMyFansViewController
...
-(void)loadData {
    ...
    [BGCommunityApi getMyFansList:param succ:^(NSDictionary *response) {
        DLog(@"\n>>>[getMyFansList sucess]:%@",response);
        ...
        DLog(@"\n>>>[getMyFansList failure]:%@",response);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([Tool arrayIsNotEmpty:_cellDataArr]) {
        ...
        [cell updataWithArray:model listType:1];
         __weak __typeof(self) weakSelf = self;
        cell.concernClicked = ^(UIButton *sender) {
