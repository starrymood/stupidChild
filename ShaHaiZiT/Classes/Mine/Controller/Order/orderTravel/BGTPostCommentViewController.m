//
//  BGTPostCommentViewController.m
//  shzTravelC
//
//  Created by biao on 2018/8/27.
...
@interface BGTPostCommentViewController ()<HXPhotoViewDelegate,UIImagePickerControllerDelegate,UITextViewDelegate>
...
@property (nonatomic, copy) NSString *starZeroStr;
...
@implementation BGTPostCommentViewController
...
-(void)loadSubViews{
    ...
    XHStarRateView *starRateView0 = [[XHStarRateView alloc] initWithFrame:CGRectMake(121, 67, 150, 20) isT:YES finish:^(CGFloat currentScore) {
        self.starZeroStr = [NSString stringWithFormat:@"%d",(int)currentScore];
    }];
    [self.orderInfoView addSubview:starRateView0];
    ...
    [self.picImgView sd_setImageWithURL:[NSURL URLWithString:_main_picture] placeholderImage:BGImage(@"img_placeholder")];
   
    self.titleeLabel.text = _titleeStr;
}

- (IBAction)btnUploadCommentClicked:(UIButton *)sender {
    if ([Tool isBlankString:_starZeroStr]) {
        [WHIndicatorView toast:@"请给满意程度评分"];
        return;
    }
    if ([Tool isBlankString:_starOneStr]) {
        [WHIndicatorView toast:@"请给准时程度评分"];
        return;
    }
    if ([Tool isBlankString:_starTwoStr]) {
        [WHIndicatorView toast:@"请给路程合理评分"];
        return;
    }
...
-(void)publishPostActionWithArr:(NSMutableArray *)arr{
    ...
    [param setObject:_starZeroStr forKey:@"satisfaction_level"];
