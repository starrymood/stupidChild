//
//  BGOrderOneDetailViewController.m
//  ShaHaiZiT
//
...
@implementation BGOrderOneDetailViewController
...
-(void)updateSubViewsWithModel:(BGOrderDetailModel *)model{
    ...
    self.navigationItem.title = [NSString stringWithFormat:@"%@%@%@",model.region_name,model.airport_name,model.product_set_name];
    self.orderNumLabel.text = [NSString stringWithFormat:@"订单号：%@",model.order_number];
    NSString *startStr = [Tool dateFormatter:model.start_time.doubleValue dateFormatter:@"yyyy-MM-dd HH:mm"];
    NSString *weakStr = [Tool getWeekDayFordate:model.start_time.doubleValue];
    self.dateLabel.text = [NSString stringWithFormat:@"%@  %@",startStr,weakStr];
    self.originLabel.text = model.airport_name;
    self.flightNumLabel.text = model.flight_number;
