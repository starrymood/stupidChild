...
@interface BGAllOrderViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation BGAllOrderViewController
-(UIView *)noneView {
    if (!_noneView) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    __weak __typeof(self) weakSelf = self;
    cell.firstBtnClicked = ^{
        [weakSelf btnFirstBtnClickedActionWithModel:model];
    };
    
    cell.sureBtnClicked = ^{
        [weakSelf btnSureBtnClickedActionWithModel:model];
    };
...
-(void)btnFirstBtnClickedActionWithModel:(BGOrderModel *)model{
    int type = model.status.intValue;
    switch (type) {
        case 1:{
        ...
        case 3:{  // 查看物流
            ...
            webVC.url = [NSString stringWithFormat:@"%@%@%@",BGWebMainHtml,@"order_logistics.html?orderid=",model.order_number];
...
-(void)btnSureBtnClickedActionWithModel:(BGOrderModel *)model{
    int type = model.status.intValue;
    switch (type) {
        case 1:{  
        }
        ...
        case 5:{   // 评价订单
            ...
            postVC.orderItems = [NSArray arrayWithArray:model.goods_list];
