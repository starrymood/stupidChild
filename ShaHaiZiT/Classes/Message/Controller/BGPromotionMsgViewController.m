//
//  BGPromotionMsgViewController.m
//  ShaHaiZiT
//
...
@interface BGPromotionMsgViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation BGPromotionMsgViewController
...
- (void)loadSubViews {
    ...
    self.navigationItem.title = @"我的活动消息";
...
- (void)loadData {
    ...
    [BGMemberApi getActivityMessage:param succ:^(NSDictionary *response) {
        DLog(@"\n>>>[getActivityMessage sucess]:%@",response);
        ...
        DLog(@"\n>>>[getActivityMessage failure]:%@",response);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model.state.intValue == 2) {
        ...
        webVC.activityTitleStr = model.title;
        webVC.isShowActivityShare = YES;
        webVC.subTitleStr = model.activity_subtitle;
        webVC.service_id = model.service_id;
        webVC.service_name = model.service_name;
